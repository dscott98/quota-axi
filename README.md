...
| MiniMax (opt-in) | `$MINIMAX_API_KEY` first, then opencode `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `$LOCALAPPDATA/opencode/auth.json` on Windows when available, then `~/.local/share/opencode/auth.json`) for a literal key under `minimax` or `minimax-coding-plan`; Pi's `$PI_CODING_AGENT_DIR/auth.json` `minimax` entry (literal `api_key.key`) is consulted after opencode. Opt-in: not in the default probe list.                                           |
