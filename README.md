...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity (`agy`), Command Code, MiniMax, MiMo, DeepSeek, OpenRouter, and ElevenLabs quota windows in one [AXI](https://axi.md)-shaped call. MiniMax is opt-in via `--provider minimax` and is excluded from the default probe.
...
| MiniMax (opt-in)       | Reports the supplied per-model `model:<name>:interval` and `model:<name>:weekly` Coding Plan meters from valid remaining percentages or count totals. It normalizes supplied reset timestamps and formats remaining durations as text; see [MiniMax provider notes](#minimax-opt-in) for counter interpretation and validation. The vendor does not establish whether per-model meters are independent or jointly bound, so effective availability stays unknown.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| MiniMax (opt-in) | See [MiniMax provider notes](#minimax-opt-in) for credential sources, paths, and accepted record shapes. |
...
- For each credential probed, MiniMax's opt-in adapter sends one no-cost, redirect-disabled `GET`: global Coding Plan keys use `https://api.minimax.io/v1/api/openplatform/coding_plan/remains`; native CLI and China Coding Plan credentials use `/v1/token_plan/remains`; `sk-api-` keys use `/account/query_balance` and report `credits.remaining` in USD globally or CNY in China. Requests run with a 15 second per-request deadline and a 262,144-byte decoded-body cap. It reports the vendor's per-model time- or count-metered allowance without calling inference or deriving quota from model lists.
- It checks four independent sources in a fixed order: `$MINIMAX_API_KEY` first (explicit caller intent), then OpenCode `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `$LOCALAPPDATA/opencode/auth.json` on Windows when available, otherwise `~/.local/share/opencode/auth.json`), then Pi's `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`), for a literal key. OpenCode checks `minimax` then `minimax-coding-plan`, trying the next candidate only after definitive rejection; Pi reads only `minimax`. OpenCode records must have `type: "api"` and a literal `key`; Pi records must have `type: "api_key"` and a literal `key`. Finally it reads `$MMX_CONFIG_DIR/config.json` (default `~/.mmx/config.json`, capped at 64 KiB), trying native `oauth.access_token` then `api_key` through the same credential selector. `$MINIMAX_BASE_URL` selects the global or China first-party host; native configuration prefers `base_url`, then `oauth.resource_url`, then `region: cn`, then the environment. Only HTTPS `api.minimax.io` and `api.minimaxi.com` origins are accepted.
- HTTP 401/403 and vendor `base_resp.status_code: 1004` reject the credential; other nonzero vendor status codes are request failures, and HTTP 429 is rate-limited. Request, transport, timeout, and decoding failures stop credential handover. The interval and weekly `usage_count` fields report remaining requests, so count-based remaining percentages are `usage_count / total_count * 100`. Valid vendor meters produce windows only from supplied remaining percentages or coherent counters: out-of-range percentages and over-total or inconsistent counters are named in `state.untrustedWindowIds` rather than clamped. Valid supplied `start_time` and `weekly_start_time` are preserved for per-window pace. Time-metered reset timestamps accept epoch seconds or milliseconds, and vendor remaining durations are milliseconds.
