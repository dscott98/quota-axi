...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity (`agy`), and Command Code quota windows in one [AXI](https://axi.md)-shaped call. MiniMax is also supported as an opt-in provider selectable with `--provider minimax`; it never appears in a default probe.
...
| MiniMax (opt-in)    | `$MINIMAX_API_KEY` first, then opencode `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `~/.local/share/opencode/auth.json`) for a literal key under `minimax` or `MiniMax`; Pi's `$PI_CODING_AGENT_DIR/auth.json` `minimax` / `MiniMax` entry (literal `api_key.key` or `oauth.access`) is consulted after opencode. Opt-in: not in the default probe list.                                                                                |
...
- MiniMax ships no first-party quota or billing endpoint, so this adapter reports auth usability rather than invented percentages. A successful no-cost probe against `https://api.MiniMax.chat/v1/models` confirms the key still authorises the account; the response body is discarded, and quota figures are never derived from model lists.
- It checks three independent sources in a fixed order: `$MINIMAX_API_KEY` first (explicit caller intent), then opencode `auth.json` for a literal key under `minimax` or `MiniMax`, then Pi's `$PI_CODING_AGENT_DIR/auth.json` for a literal `api_key.key` or `oauth.access` under the same ids. A Pi OAuth record is accepted only when its `access` is a usable literal bearer; stored expiry is advisory and the record is still probed.
- Each source sends one redirect-disabled `GET` to `/v1/models` with a 15 second total deadline and a 262,144-byte decoded-body cap. Only HTTP 401 or 403 is a definitive auth rejection for that key; HTTP 429 is rate-limited, every other non-2xx status and every transport/timeout/decoding failure is a non-credential error and stays usable for stale-cache eligibility on the same reading.
- A live reading publishes `authStatus: usable` with `windows: []`. A definitively rejected credential becomes `auth_required` and retires the MiniMax cache; an absent or unreadable store is reported as `auth_required` only when every other source is also absent, mirroring the cross-provider rule that absence alone never degrades a sibling source.
...
- OpenRouter publishes a single bounded, read-only `GET /api/v1/auth/key` endpoint that returns the key's `label`, `usage`, and (when set) its `limit`. quota-axi reports exactly that: a `credits` window with `spentUsd` and `limitUsd` when `data.limit` is a positive number, and `authStatus: usable` with empty windows when `data.limit` is `null` (free tier or unlimited). A numeric `usage` without a positive `limit` never becomes a percentage - matching the Grok Pi OAuth "live_no_quota" path.
...
- Each source sends one redirect-disabled `GET` to `/api/v1/auth/key` with a 15 second total deadline and a 262,144-byte decoded-body cap. Only HTTP 401 or 403 is a definitive auth rejection for that key; HTTP 429 is rate-limited, every other non-2xx status and every transport/timeout/decoding failure is a non-credential error and stays usable for stale-cache eligibility on the same reading.
...
- A definitively rejected credential becomes `auth_required` and retires the OpenRouter cache; an absent or unreadable store is reported as `auth_required` only when every other source is also absent, mirroring the cross-provider rule that absence alone never degrades a sibling source.
- A structurally invalid present-but-broken entry is named `credentialPresent: true` so a working sibling is never hidden behind it.
