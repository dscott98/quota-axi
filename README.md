...
| MiniMax (opt-in)    | `$MINIMAX_API_KEY` first, then opencode `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `~/.local/share/opencode/auth.json`) for a literal key under `minimax`, `MiniMax`, or `minimax-coding-plan`; Pi's `$PI_CODING_AGENT_DIR/auth.json` entry under the same ids (literal `api_key.key` or `oauth.access`) is consulted after opencode. Opt-in: not in the default probe list.                                           |
...
- MiniMax's opt-in Coding Plan adapter sends one no-cost, redirect-disabled `GET` to `https://api.minimax.io/v1/api/openplatform/coding_plan/remains` with a 15 second total deadline and a 262,144-byte decoded-body cap. It reports the vendor's per-model time- or count-metered allowance without calling inference or deriving quota from model lists.
...
- HTTP 401/403 and a nonzero vendor `base_resp.status_code` are rejected credential or request failures; HTTP 429 is rate-limited. Valid vendor meters produce windows only from supplied remaining percentages or counters. Time-metered reset timestamps accept epoch seconds or milliseconds, and vendor remaining durations are milliseconds.
