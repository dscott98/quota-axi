...
- Each live card with a combinable bound leads with the effective-availability rollup (min across bounding windows), colored by headroom: >=50% healthy, 20-50% tight, <20% critical. Per-window rows, including per-model breakouts, are the supporting detail. The row label column widens per card from 8 up to 16 display cells to fit that card's own labels (MiniMax's `general interval`/`general weekly`, Alibaba's `model:qwen3-coder-plus`), so distinct meters never collapse into identical ellipses; the bar gives back exactly the borrowed cells and short-label cards keep the fixed layout.
...
| OpenRouter             | Reads a local OpenRouter/Pi API key plus the first-party key and credits endpoints. A finite key cap is reported as a raw `key-limit` credit amount; a capless key reports the purchased-credit balance (`total_credits`/`total_credits_purchased` minus `total_usage`/`total_credits_used`) as `credits.remaining` in `usd`. A null cap means no spend limit was configured, never unlimited credit, so OpenRouter never reports `credits.unlimited`, and no model headroom is inferred. An unreadable balance leaves the reading fresh without a credits field.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
