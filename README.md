...
| MiniMax (opt-in)    | `$MINIMAX_API_KEY` first, then opencode `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `~/.local/share/opencode/auth.json`) for a literal key under `minimax`, `MiniMax`, or `MiniMax`; Pi's `$PI_CODING_AGENT_DIR/auth.json` `minimax` / `MiniMax` entry (literal `api_key.key` or `oauth.access`) is consulted after opencode. Opt-in: not in the default probe list.                                                                                |
...
- It checks three independent sources in a fixed order: `$MINIMAX_API_KEY` first (explicit caller intent), then opencode `auth.json` for a literal key under `minimax`, `MiniMax`, or `MiniMax`, then Pi's `$PI_CODING_AGENT_DIR/auth.json` for a literal `api_key.key` or `oauth.access` under the same ids. A Pi OAuth record is accepted only when its `access` is a usable literal bearer; stored expiry is advisory and the record is still probed.
