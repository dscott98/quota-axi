...
- A live reading with no supplied meter publishes `authStatus: usable` with `windows: []`. When credential rejection defines the provider verdict, it reports `auth_required`. Absent sources produce `auth_required` only when every source is absent; an unreadable source remains an error rather than being treated as absent, mirroring the cross-provider rule that absence alone never degrades a sibling source.
