...
- HTTP 401/403 and vendor `base_resp.status_code: 1004` reject the credential; other nonzero vendor status codes are request failures, and HTTP 429 is rate-limited. Valid vendor meters produce windows only from supplied remaining percentages or coherent counters: out-of-range percentages and over-total or inconsistent counters are named in `state.untrustedWindowIds` rather than clamped. Time-metered reset timestamps accept epoch seconds or milliseconds, and vendor remaining durations are milliseconds.
- A live reading with no supplied meter publishes `authStatus: usable` with `windows: []`. A definitively rejected credential becomes `auth_required` and retires the MiniMax cache. Absent sources produce `auth_required` only when every source is absent; an unreadable source remains an error rather than being treated as absent, mirroring the cross-provider rule that absence alone never degrades a sibling source.
...
- A definitively rejected credential becomes `auth_required` and retires the OpenRouter cache. Absent sources produce `auth_required` only when every source is absent; an unreadable source remains an error rather than being treated as absent, mirroring the cross-provider rule that absence alone never degrades a sibling source.
- A structurally invalid present-but-broken entry is named `credentialPresent: true` so a working sibling is never hidden behind it.
