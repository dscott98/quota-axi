...
| MiniMax (opt-in) | `$MINIMAX_API_KEY` first, then opencode `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `$LOCALAPPDATA/opencode/auth.json` on Windows when available, then `~/.local/share/opencode/auth.json`) for a literal key under `minimax`, `MiniMax`, or `minimax-coding-plan`; Pi's `$PI_CODING_AGENT_DIR/auth.json` entry under the same ids (literal `api_key.key`) is consulted after opencode. Opt-in: not in the default probe list.                      |
...
- It checks three independent sources in a fixed order: `$MINIMAX_API_KEY` first (explicit caller intent), then opencode `auth.json`, then Pi's `$PI_CODING_AGENT_DIR/auth.json`, for a literal key under `minimax`, `MiniMax`, or `minimax-coding-plan`. OpenCode records must have `type: "api"` and a literal `key`; Pi records must have `type: "api_key"` and a literal `key`.
