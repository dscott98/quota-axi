...
auth[28]{provider,source,path,status,error}:
...
MiniMax is opt-in for quota (including `--tui`) and `auth`: select it with `--provider minimax`, or include it in a comma-separated provider list. The `models` command uses its own [catalog-backed defaults](#model-catalog-and-models).
...
- Each live card with a combinable bound leads with the effective-availability rollup (min across bounding windows), colored by headroom: >=50% healthy, 20-50% tight, <20% critical. Per-window rows, including per-model breakouts, are the supporting detail. The row label column widens per card from 8 up to 16 display cells, with a trailing space before the bar. Labels such as `general interval` and `general weekly` fit in full; longer labels may still be shortened. The bar shrinks to keep the card width fixed.
...
| Alibaba                | Reads the local `bl` CLI's Alibaba Coding Plan Token Plan usage; reports the plan name and weekly remaining percentage and reset time from the CLI's JSON output, plus any named model limits as separate `model:<name>` windows. Repeated limits for the same model remain separate with suffixed IDs such as `model:<name>:2`. A `bl` failure that exits non-zero with the CLI's structured JSON error on stderr is classified from that payload: the observed console-session failure (`error.code` 3, "Console session is not logged in or has expired") reports `auth_required` with error `bl_console_session_expired` and the remedy `bl auth login --console`; any other structured message is reported as that single-line message instead of the raw multi-line command blob, and unparseable failures keep the bounded `bl_usage_failed` form.                                                                                                                                                     |
...
| OpenRouter             | Reads a local OpenRouter/Pi API key plus the first-party key and credits endpoints. A finite key cap with a remaining amount produces a `key-limit` window with a remaining percentage and reports that amount as `credits.remaining` in `usd`. When the key supplies no usable remaining amount or has no cap, the purchased-credit balance (`total_credits`/`total_credits_purchased` minus `total_usage`/`total_credits_used`, clamped at zero without rounding) supplies `credits.remaining` instead. A null cap means no spend limit was configured, never unlimited credit, so OpenRouter never reports `credits.unlimited`, and no model headroom is inferred. An unreadable balance leaves the reading fresh without a credits field.                                                                                                                                                                                                                                                                 |
...
- It sends one redirect-disabled `GET` per attempted credential to OpenRouter's first-party `/api/v1/key` endpoint with a bearer token, followed by one `GET` to the documented first-party `/api/v1/credits` endpoint only when the key reading does not supply a remaining amount under a configured cap, trying the environment key before Pi under the [credential handover rules](#provider-state). A credits-read failure never downgrades the already-proven key reading; the balance simply stays unreported. Key-cap and balance representation is documented under [Provider windows](#provider-windows); reset text comes only from the endpoint. The endpoint's `data.label` is a per-key nickname rather than an account identifier, so it is exposed as `account.accountId` with `identityStatus` `unverified` in `--full` output.
