 ...
 * opt-in adapters (`minimax`) that are not part of the default probe set, so
 * the selector itself stays the single source of truth for what the CLI is
 * willing to talk to.
 */
 ...
 * explicitly with `--provider minimax` (or any single name from
 * {@link PROVIDER_IDS}).
 */
